...
let handler = async (m, { conn, args, usedPrefix, text }) => {
    ...
    try {
    let Puki = `https://api.yanzbotz.my.id/api/ai/bardimg?query=${encodeURIComponent(text)}&url=${encodeURIComponent(url)}&apiKey=${global.yanz}`
    let { data } = await axios.get(Puki)
